...
async function createOrder(req, res) {
    try {
        ...
        let plan = await prisma_1.default.membershipPlan.findUnique({
            where: { tier: planTier },
        });
        if (!plan) {
            ...
            plan = await prisma_1.default.membershipPlan.create({
                data: {
                    tier: planTier,
                    name: planNames[planTier],
                    price: planPrices[planTier],
                    durationDays: 30,
                    features: features[planTier].join(", "),
                },
            });
        }
...
async function verifyPayment(req, res) {
    try {
        ...
        const plan = await prisma_1.default.membershipPlan.findUnique({ where: { id: planId } });
        if (!plan) {
