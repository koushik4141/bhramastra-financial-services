...
exports.getMarketTickerData = getMarketTickerData;
...
async function fetchFromYahoo() {
    ...
    const quotes = rawQuotes.filter((q) => q !== null);
 ...
 */
async function getMarketTickerData() {
    const requestStartTime = Date.now();
    // 1. Check cache first
    const cachedData = cache.get(CACHE_KEY);
    if (cachedData) {
        console.log(`[MarketService] Cache HIT. Request resolved in ${Date.now() - requestStartTime}ms`);
        return {
            success: true,
    ...
    if (fetchedData && fetchedData.length > 0) {
        ...
        console.log(`[MarketService] Data cached successfully. Total request time: ${Date.now() - requestStartTime}ms`);
        return {
            success: true,
            lastUpdated: new Date().toISOString(),
            data: fetchedData,
        ...
        console.warn(`[MarketService] Yahoo Finance unavailable. Returning fallback data. Total time: ${Date.now() - requestStartTime}ms`);
